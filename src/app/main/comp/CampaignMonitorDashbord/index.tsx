...
//   useEffect(() => {
//     // 모든 열린 탭을 디버깅 로그로 출력
//     console.log("모든 열린 탭:", openedTabs);
    
//     // activeTabKey가 있는지 확인
//     if (activeTabKey) {
//       console.log("현재 활성 탭 키:", activeTabKey);
      
//       // 약간의 지연을 두고 실행 (상태 업데이트가 완료되길 기다림)
//       const timer = setTimeout(() => {
//         // 활성 탭 찾기
//         const foundActiveTab = openedTabs.find(tab => tab.uniqueKey === activeTabKey);
//         console.log("찾은 활성 탭:", foundActiveTab);
        
//         // 캠페인 ID 추출 및 API 호출
//         if (foundActiveTab && foundActiveTab.campaignId) {
//           const campaignId = Number(foundActiveTab.campaignId);
...
//             const tempCampaign = campaigns.find(data => data.campaign_id === campaignId);
//             console.log("찾은 캠페인 정보:", tempCampaign);
            
//             if (tempCampaign) {
//               setCampaignInfo(tempCampaign);
//               setCampaignIdList([campaignId]);
//             }
//           }
//         } else {
//           console.warn("유효한 campaignId가 없어서 API 호출을 건너뜁니다.");
          
//           // 폴백: URL 파라미터나 다른 소스에서 캠페인 ID 확인
//           const urlParams = new URLSearchParams(window.location.search);
//           const urlCampaignId = urlParams.get('campaignId');
          
//           if (urlCampaignId) {
//             const numericCampaignId = Number(urlCampaignId);
//             console.log("URL에서 추출한 캠페인 ID:", numericCampaignId);
...
//     }
//   }, [activeTabKey, openedTabs, campaigns, fetchCampaignProgressInformation]);
