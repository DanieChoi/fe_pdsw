...
const CampaignManager = ({ campaignId, isOpen, onCampaignPopupClose }: Props) => {
  ...
  useEffect(() => {
    ...
    if (campaigns.length > 0 && selectedCampaign != null) {
      setSelectedCampaign(
        campaigns.find(c => c.campaign_id === Number(selectedCampaign.campaign_id)) || null
      );
    }
  }, [tenants, campaigns, selectedCampaign]);
  ...
  return (
    <div className='compaign-wrap stable-scrollbar' style={{
      ...
      <div className='flex flex-col gap-[15px] limit-width'>
        <CampaignManagerHeader campaignId={masterCampaignId} onSearch={handleCampaignHeaderSearch} />
        <div className="flex gap-[30px]">
          <CampaignManagerList
