...
export interface OutgoingMethodTabParam {
  ...
  alarm_answer_count: number;     //콜 목표량
  overdial_abandon_time: number;  //포기호 처리시간(초)
  detect_mode: number;            //기계음 처리
  auto_dial_interval: number;     //자동 다이얼 시
  power_divert_queue: number;     //연결 IVR NO 및 다이얼 모드
  next_campaign: number;          //연결 캠페인
  DDD_code: string;               //DDD Number
  callback_kind: number;          //연결구분 - 콜백구분
  max_ring: number;               //최대 링 횟수
  token_id: number;               //토큰 ID
  use_counsel_result: number;     //상담결과 등록 여부
  dial_mode_option: number;       //다이얼 모드 옵션
  user_option: string;            //제한 호수 비율
}
...
export default function CampaignDetail() {
  ...
  const { tenants, setCampaigns, selectedCampaign, setSelectedCampaign } = useMainStore();
  ...
  // ===== useEffect로 selectedCampaign의 변경 시 temp states 업데이트 =====
  useEffect(() => {
    // 1) selectedCampaign 없으면 아무것도 안 함
    if (!selectedCampaign) return;

    // 2) tempCampaignInfo 업데이트 (함수형 업데이트 + optional chaining)
    setTempCampaignsInfo(prev => ({
      ...prev,
      campaign_id: selectedCampaign?.campaign_id ?? 0,
      campaign_name: selectedCampaign?.campaign_name ?? '',
      campaign_desc: selectedCampaign?.campaign_desc ?? '',
      site_code: selectedCampaign?.site_code ?? 0,
      service_code: selectedCampaign?.service_code ?? 0,
      start_flag: selectedCampaign?.start_flag ?? 0,
      end_flag: selectedCampaign?.end_flag ?? 0,
      dial_mode: selectedCampaign?.dial_mode ?? 0,
      callback_kind: selectedCampaign?.callback_kind ?? 0,
      delete_flag: selectedCampaign?.delete_flag ?? 0,
      list_count: selectedCampaign?.list_count ?? 0,
      list_redial_query: selectedCampaign?.list_redial_query ?? '',
      next_campaign: selectedCampaign?.next_campaign ?? 0,
      token_id: selectedCampaign?.token_id ?? 0,
      phone_order: selectedCampaign?.phone_order ?? '',
      phone_dial_try: selectedCampaign?.phone_dial_try ?? [],
      dial_try_interval: selectedCampaign?.dial_try_interval ?? 0,
      trunk_access_code: selectedCampaign?.trunk_access_code ?? '',
      DDD_code: selectedCampaign?.DDD_code ?? '',
      power_divert_queue: selectedCampaign?.power_divert_queue ?? 0,
      max_ring: selectedCampaign?.max_ring ?? 0,
      detect_mode: selectedCampaign?.detect_mode ?? 0,
      auto_dial_interval: selectedCampaign?.auto_dial_interval ?? 0,
      creation_user: selectedCampaign?.creation_user ?? '',
      creation_time: selectedCampaign?.creation_time ?? '',
      creation_ip: selectedCampaign?.creation_ip ?? '',
      update_user: selectedCampaign?.update_user ?? '',
      update_time: selectedCampaign?.update_time ?? '',
      update_ip: selectedCampaign?.update_ip ?? '',
      dial_phone_id: selectedCampaign?.dial_phone_id ?? 0,
      tenant_id: selectedCampaign?.tenant_id ?? 0,
      alarm_answer_count: selectedCampaign?.alarm_answer_count ?? 0,
      dial_speed: selectedCampaign?.dial_speed ?? 0,
      parent_campaign: selectedCampaign?.parent_campaign ?? 0,
      overdial_abandon_time: selectedCampaign?.overdial_abandon_time ?? 0,
      list_alarm_count: selectedCampaign?.list_alarm_count ?? 0,
      supervisor_phone: selectedCampaign?.supervisor_phone ?? '',
      reuse_count: selectedCampaign?.reuse_count ?? 0,
      use_counsel_result: selectedCampaign?.use_counsel_result ?? 0,
      use_list_alarm: selectedCampaign?.use_list_alarm ?? 0,
      redial_strategy: selectedCampaign?.redial_strategy ?? [],
      dial_mode_option: selectedCampaign?.dial_mode_option ?? 0,
      user_option: selectedCampaign?.user_option ?? '',
    }));
    ...
    const tempSkill = campaignSkills
      ...
      .join(',');
    setInputSkills(tempSkill);
    setTempCampaignSkills(prev => ({
      ...prev,
      campaign_id: selectedCampaign?.campaign_id ?? 0,
      skill_id: tempSkill ? tempSkill.split(',').map(Number) : [],
    }));

    // 4) 발신번호
    const tempCallNumber = callingNumbers
      .filter(c => c.campaign_id === selectedCampaign.campaign_id)
      .map(data => data.calling_number)
      .join(',');
    setInputCallingNumber(tempCallNumber);
    setTempCallingNumberInfo(prev => ({
      ...prev,
      campaign_id: selectedCampaign?.campaign_id ?? 0,
      calling_number: tempCallNumber,
    }));
    ...
    // 5) 발신속도
    setTempCampaignDialSpeedInfo(prev => ({
      ...prev,
      campaign_id: selectedCampaign?.campaign_id ?? 0,
      tenant_id: selectedCampaign?.tenant_id ?? 0,
      dial_speed: selectedCampaign?.dial_speed ?? 0,
    }));
    setTempCampaignDialSpeedInfoParam(prev => ({
      ...prev,
      dial_mode: selectedCampaign?.dial_mode ?? 0,
      predictive_dial_speed:
        selectedCampaign?.dial_mode === 2 ? 0 : (selectedCampaign?.dial_speed ?? 0),
      progressive_dial_speed:
        selectedCampaign?.dial_mode === 3 ? 0 : (selectedCampaign?.dial_speed ?? 0),
    }));

    // 6) CampaignManagerInfo
    setTempCampaignManagerInfo(prev => ({
      ...prev,
      campaign_id: selectedCampaign?.campaign_id ?? 0,
      campaign_name: selectedCampaign?.campaign_name ?? '',
      campaign_desc: selectedCampaign?.campaign_desc ?? '',
      ...
      token_id: selectedCampaign?.token_id ?? 0,
      phone_order: selectedCampaign?.phone_order ?? '',
      phone_dial_try1: selectedCampaign?.phone_dial_try
        ? Number(selectedCampaign.phone_dial_try.slice(0, 1)[0])
        : 0,
      phone_dial_try2: selectedCampaign?.phone_dial_try
        ? Number(selectedCampaign.phone_dial_try.slice(1, 2)[0])
        : 0,
      phone_dial_try3: selectedCampaign?.phone_dial_try
        ? Number(selectedCampaign.phone_dial_try.slice(2, 3)[0])
        : 0,
      phone_dial_try4: selectedCampaign?.phone_dial_try
        ? Number(selectedCampaign.phone_dial_try.slice(3, 4)[0])
        : 0,
      phone_dial_try5: selectedCampaign?.phone_dial_try
        ? Number(selectedCampaign.phone_dial_try.slice(4, 5)[0])
        : 0,
      dial_try_interval: selectedCampaign?.dial_try_interval ?? 0,
      trunk_access_code: selectedCampaign?.trunk_access_code ?? '',
      DDD_code: selectedCampaign?.DDD_code ?? '',
      power_divert_queue: String(selectedCampaign?.power_divert_queue ?? ''),
      max_ring: selectedCampaign?.max_ring ?? 0,
      detect_mode: selectedCampaign?.detect_mode ?? 0,
      auto_dial_interval: selectedCampaign?.auto_dial_interval ?? 0,
      creation_user: (selectedCampaign?.creation_user ?? '') + '',
      ...
      use_counsel_result: selectedCampaign?.use_counsel_result ?? 0,
      use_list_alarm: selectedCampaign?.use_list_alarm ?? 0,
      redial_strategy1: selectedCampaign?.redial_strategy
        ? String(selectedCampaign.redial_strategy.slice(0, 1)[0])
        : '',
      redial_strategy2: selectedCampaign?.redial_strategy
        ? String(selectedCampaign.redial_strategy.slice(1, 2)[0])
        : '',
      redial_strategy3: selectedCampaign?.redial_strategy
        ? String(selectedCampaign.redial_strategy.slice(2, 3)[0])
        : '',
      redial_strategy4: selectedCampaign?.redial_strategy
        ? String(selectedCampaign.redial_strategy.slice(3, 4)[0])
        : '',
      redial_strategy5: selectedCampaign?.redial_strategy
        ? String(selectedCampaign.redial_strategy.slice(4, 5)[0])
        : '',
      dial_mode_option: selectedCampaign?.dial_mode_option ?? 0,
      user_option: selectedCampaign?.user_option ?? '',
      customer_char_id: 1,
    ...
    if (schedules.length > 0) {
      const matched = schedules.filter(
        sch => sch.campaign_id === selectedCampaign.campaign_id
      );
      if (matched.length > 0) {
        setTempCampaignSchedule(prev => ({
          ...prev,
          ...matched[0],
          campaign_id: selectedCampaign?.campaign_id ?? 0,
          tenant_id: selectedCampaign?.tenant_id ?? 0,
  ...
  // =========================
  // input data change 핸들러
  // =========================
  const handleInputData = (value: any, col: string) => {
    // if (col === 'campaign_id') ...
    if (col === 'campaign_id' && value !== '') {
      setTempCampaignsInfo(prev => ({ ...prev, campaign_id: Number(value) }));
      setTempCampaignManagerInfo(prev => ({ ...prev, campaign_id: Number(value) }));
    }
    if (col === 'campaign_name') {
      setTempCampaignsInfo(prev => ({ ...prev, campaign_name: value }));
      setTempCampaignManagerInfo(prev => ({ ...prev, campaign_name: value }));
    }
    if (col === 'campaign_desc') {
      setTempCampaignsInfo(prev => ({ ...prev, campaign_desc: value }));
      setTempCampaignManagerInfo(prev => ({ ...prev, campaign_desc: value }));
    }
  };

  const handleSelectChange = (value: string, type: 'tenant' | 'dialMode') => {
    if (type === 'tenant' && value !== '') {
      setTempCampaignsInfo(prev => ({ ...prev, tenant_id: Number(value) }));
      setTempCampaignManagerInfo(prev => ({ ...prev, tenant_id: Number(value) }));
    }
    if (type === 'dialMode' && value !== '') {
      const newDialMode = Number(value);
      setTempCampaignsInfo(prev => ({ ...prev, dial_mode: newDialMode }));
      setTempCampaignManagerInfo(prev => ({ ...prev, dial_mode: newDialMode }));
      setTempCampaignDialSpeedInfoParam(prev => ({ ...prev, dial_mode: newDialMode }));
    }
  };

  // 스킬 선택 팝업
  const handleSelectSkills = (param: string) => {
    const oldStr = tempCampaignSkills.skill_id.join(',');
    if (oldStr !== param) {
      setCampaignSkillChangeYn(true);
      setInputSkills(param);
      setTempCampaignSkills(prev => ({
  ...
  // ====== 아래로는 각 탭 변경 핸들러들 (동작시간, 발신순서, 발신전략, 발신방법 등) ======
  const handleCampaignScheduleChange = (value: OperationTimeParam) => {
    if (value.campaignInfoChangeYn) {
      setTempCampaignManagerInfo(prev => ({ ...prev, start_flag: Number(value.start_flag) }));
      setTempCampaignsInfo(prev => ({ ...prev, start_flag: Number(value.start_flag) }));
    }
    if (value.campaignScheduleChangeYn) {
      setCampaignScheduleChangeYn(true);
      setTempCampaignSchedule(prev => ({
  ...
  const handleOutgoingStrategyTabChange = (value: OutgoingStrategyTabParam) => {
    if (value.campaignInfoChangeYn) {
      setTempCampaignsInfo(prev => ({ ...prev, redial_strategy: value.redial_strategy }));
      setTempCampaignManagerInfo(prev => ({
        ...prev,
        redial_strategy1: value.redial_strategy[0],
        redial_strategy2: value.redial_strategy[1],
    ...
    if (value.onInit) {
      // 초기화버튼
      setTempCampaignsInfo(prev => ({ ...prev, redial_strategy: CampaignInfo.redial_strategy }));
      setTempCampaignManagerInfo(prev => ({
        ...prev,
        redial_strategy1: CampaignManagerInfo.redial_strategy1,
        redial_strategy2: CampaignManagerInfo.redial_strategy2,
  ...
  const handleCallPacingTabChange = (value: CallPacingTabParam) => {
    if (value.campaignDialSpeedChangeYn) {
      ...
      setTempCampaignDialSpeedInfo(prev => ({
        ...prev,
        dial_speed: value.dial_mode === 2
          ? Math.floor(value.progressive_dial_speed)
          : value.predictive_dial_speed,
      }));
    }
    if (value.onSave) {
      handleCampaignSave();
  ...
  const handleCampaignSave = () => {
    setAlertState({
      ...
      isOpen: true,
      message:
        '캠페인 아이디 : ' + tempCampaignManagerInfo.campaign_id +
        '\n 캠페인 이름 : ' + tempCampaignManagerInfo.campaign_name +
        '\n 캠페인을 수정하시겠습니까?',
      onClose: handleCampaignSaveExecute,
      onCancle: () => setAlertState(prev => ({ ...prev, isOpen: false })),
  ...
  // ====== 예시: useEffect로 changeYn, campaignInfoChangeYn 등 처리 ======
  useEffect(() => {
    if (
      changeYn &&
  ...
  const handleListManager = () => {
    if (openedTabs.some(tab => tab.id === 7)) {
      const found = openedTabs.find(d => d.id === 7);
      if (found) setActiveTab(7, found.uniqueKey);
    } else {
      addTab({
        id: 7,
        uniqueKey: '7',
  ...
  const handleRebroadcast = () => {
    if (!campaignIdForUpdateFromSideMenu) {
      setCampaignIdForUpdateFromSideMenu(String(tempCampaignInfo.campaign_id));
    }
    if (openedTabs.some(tab => tab.id === 20)) {
      const found = openedTabs.find(d => d.id === 20);
      if (found) setActiveTab(20, found.uniqueKey);
    } else {
      addTab({
        id: 20,
        uniqueKey: '20',
  ...
  const CheckCampaignSaveReturnCode = (returnCode: number) => {
    if (returnCode === -1) {
      return 'DataBase 데이터 처리 중 문제가 발생했습니다.';
    }
    // ...생략
  };
    ...
    <div className='flex flex-col gap-5 w-[60%] overflow-auto'>
      <div>
        ...
        <div className="grid grid-cols-3 gap-x-[26px] gap-y-2">
          ...
          <div className='flex items-center gap-2'>
            ...
            <Select
              onValueChange={(value) => handleSelectChange(value, 'tenant')}
              value={String(tempCampaignInfo.tenant_id || '')}
              disabled
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="테넌트를 선택하세요" />
          ...
          <div className='flex items-center gap-2'>
            ...
            <Select
              onValueChange={(value) => handleSelectChange(value, 'dialMode')}
              value={String(tempCampaignInfo.dial_mode || '')}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="다이얼 모드를 선택하세요" />
              </SelectTrigger>
          ...
          <div className='flex items-center gap-2 relative'>
            ...
            <button
              className="absolute right-2 top-[52%] transform -translate-y-1/2"
              onClick={() => setSkillPopupState(prev => ({ ...prev, isOpen: true }))}
            >
              <Image
                src="/skill-popup.svg"
                alt="스킬팝업"
          ...
          <div className='flex items-center gap-2'>
            ...
            <CommonButton
              variant="outline"
              className='h-[24px]'
              onClick={() => setCallingNumberPopupState(prev => ({ ...prev, isOpen: true }))}
            >
              발신번호 변경
            </CommonButton>
          </div>
      ...
      <div>
        <CampaignTab
          ...
          campaignInfo={tempCampaignInfo}
          campaignDialSpeedInfo={tempCampaignDialSpeedInfoParam}
          onCampaignOutgoingOrderChange={handleCampaignOutgoingOrderChange}
          onCampaignScheduleChange={handleCampaignScheduleChange}
          onCampaignOutgoingStrategyChange={handleOutgoingStrategyTabChange}
          onCampaignOutgoingMethodChange={handleOutgoingMethodTabChange}
          onHandleCallPacingTabChange={handleCallPacingTabChange}
          onHandleAdditionalInfoTabChange={handleAdditionalInfoTabChange}
          onHandleCallbackTabChange={handleCallbackTabChange}
          onHandleNotificationTabChange={handleNotificationTabChange}
        />
      </div>
      ...
      <CallingNumberPopup
        ...
        type={callingNumberPopupState.type}
        isOpen={callingNumberPopupState.isOpen}
        onConfirm={handleCallingNumlber}
        onCancle={() => setCallingNumberPopupState(prev => ({ ...prev, isOpen: false }))}
      />
    </div>
  );
