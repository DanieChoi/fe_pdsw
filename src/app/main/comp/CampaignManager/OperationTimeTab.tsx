...
const OperationTimeTab: React.FC<Props> = ({
  ...
  const [isPassedVaildationTestForStartAndEndTime, setIsPassedVaildationTestForStartAndEndTime] = useState(false);
  ...
  const validateStartAndEndTime = () => {
    const rawStart = startTime.replace(":", "");
    const rawEnd = endTime.replace(":", "");

    const isValidFormat = rawStart.length === 4 && rawEnd.length === 4;

    const startHour = Number(rawStart.substring(0, 2));
    const startMinute = Number(rawStart.substring(2, 4));
    const endHour = Number(rawEnd.substring(0, 2));
    const endMinute = Number(rawEnd.substring(2, 4));
    ...
    const isTimeOrderCorrect = Number(rawStart) <= Number(rawEnd);

    setIsPassedVaildationTestForStartAndEndTime(
      isValidFormat && isValidStart && isValidEnd && isTimeOrderCorrect
    );

    if (!isValidStart || !isValidEnd) {
      toast.warning("시간 형식이 올바르지 않습니다. HH:MM 형식으로 입력해주세요.");
    }
  };
  ...
  const handleAddTime = () => {
    ...
    if (!check) {
      ...
      setStartTime("");
      setEndTime("");
      setIsPassedVaildationTestForStartAndEndTime(false);
    }
  };
  ...
  const handleDelete = (no: number) => {
    setTempData((prevData) => {
      ...
      const newStartTimes = newData.map((item) => item.startTime.replace(":", ""));
      const newEndTimes = newData.map((item) => item.endTime.replace(":", ""));
      const updatedSchedule = {
        ...tempCampaignSchedule,
        changeYn: true,
        campaignScheduleChangeYn: true,
              ...
              <div className="flex flex-col gap-y-2">
                <div className="flex items-center gap-[10px] justify-between">
                  ...
                  <CustomInputForTime
                    value={startTime}
                    onChange={(value) => setStartTime(value)}
                    onBlur={validateStartAndEndTime}
                  />
                </div>
                <div className="flex items-center gap-[10px] justify-between">
                  <Label className="w-[70px] min-w-[70px]">종료시간</Label>
                ...
                <div className="flex justify-end">
                  <CommonButton
                    ...
                    disabled={!isPassedVaildationTestForStartAndEndTime}
                  >
                    시간추가
                    <Image src="/addArrow.svg" alt="화살표" width={10} height={10} />
                  </CommonButton>
