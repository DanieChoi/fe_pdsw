...
import { CalendarForSettingCampaignStartAndEndDate } from "@/components/shared/CalendarForSettingCampaignStartAndEndDate";
...
const OperationTimeTab: React.FC<Props> = ({
  ...
  const parseStringToDate = (dateStr: string): Date | null => {
    ...
    const month = parseInt(dateStr.substring(4, 6)) - 1; // month는 0부터 시작
  ...
  useEffect(() => {
    if (campaignInfo && campaignSchedule.start_date !== "") {
      ...
      // Zustand store에 날짜 설정
      const startDateObj = parseStringToDate(tempCampaignScheduleData.start_date);
      const endDateObj = parseStringToDate(tempCampaignScheduleData.end_date);
  ...
  }, [callCampaignMenu, campaignSchedule, campaignInfo, setStartDate, setEndDate]);
  ...
  const handleStartDateChange = (date: Date | null) => {
    setStartDate(date);
    const startDateStr = formatDateToString(date);
    let endDateStr = tempCampaignSchedule.end_date;
  ...
  const handleEndDateChange = (date: Date | null) => {
    ...
    setEndDate(date);
    const endDateStr = formatDateToString(date);
      ...
      <div className="flex gap-[30px]">
        <div className="w-[30%]">
          ...
          <div className="flex flex-col gap-y-2">
            ...
            <div className="flex items-center gap-[10px] justify-between">
              <Label className="w-[70px] min-w-[70px]">시작 날짜</Label>
              <CalendarForSettingCampaignStartAndEndDate
                date={startDate}
