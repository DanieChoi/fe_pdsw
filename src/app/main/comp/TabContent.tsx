...
const CachedTab = ({ tabId, params, campaignId, campaignName, isVisible }: CachedTabProps) => {
  ...
  // 컴포넌트 마운트 시간 로깅
  useEffect(() => {
    console.log(`Tab ${tabId} mounted at ${initialRenderTimeRef.current}`);
  }, [tabId]);
  
  // 탭 컴포넌트에 전달할 props 준비
  ...
  }, [campaignId, campaignName, params]);

  // 보이지 않을 때는 display:none으로 숨기지만 마운트는 유지
  const style = {
  ...
  const renderTabContent = () => {
    ...
    switch (tabId) {
      case 1:
        ...
        />;
      case 2:
        return <CampaignManager campaignId={params?.campaignId} />;
      case 3:
        return <IntegratedMonitoringDashboard />;
      case 4:
        return <Campaignprogress />;
...
// 최적화된 섹션 컨텐츠 컴포넌트
const OptimizedSectionContent = React.memo(({
  section,
  rowId,
  width,
  activeTabId,
  ...
  // 섹션 내 모든 탭의 컴포넌트를 렌더링하고 CSS로 표시/숨김 처리
  const tabComponents = useMemo(() => {
    return section.tabs.map(tab => {
      const isActive = tab.uniqueKey === section.activeTabKey;
      ...
      return (
        <MemoizedCachedTab
          ...
          campaignName={tab.campaignName}
          isVisible={isActive}
        />
      );
    });
  }, [section.tabs, section.activeTabKey]);
...
const ResizeDivider = React.memo(({ handleMouseDown }: ResizeDividerProps) => {
  return (
    <div
      className="w-1 bg-gray-200 hover:bg-[#55BEC8] active:bg-[#55BEC8] cursor-col-resize select-none"
      onMouseDown={handleMouseDown}
...
const TabContent = () => {
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    setIsDragging(true);
    setResizing(true); // 전역 리사이징 상태 업데이트
    startXRef.current = e.clientX;
    document.body.style.cursor = 'col-resize';
    document.body.style.userSelect = 'none'; // Prevent text selection while dragging
  }, [setResizing]);
  ...
  const handleMouseUp = useCallback(() => {
    setIsDragging(false);
    setResizing(false); // 전역 리사이징 상태 업데이트
    document.body.style.cursor = 'default';
    document.body.style.userSelect = 'auto';
  }, [setResizing]);
        ...
        >
          {sections.map((section, index) => (
            <React.Fragment key={section.id}>
              ...
              {index === 0 && sections.length === 2 && (
                <ResizeDivider handleMouseDown={handleMouseDown} />
              )}
            </React.Fragment>
          ))}
        </div>
