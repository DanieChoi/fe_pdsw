...
export function useApiForGetTreeMenuDataForSideMenu(): TreeMenuQueryResult {
  ...
  useEffect(() => {
    if (
      ...
      previousDataRef.current = { tenants: [...tenants], campaigns: [...campaigns] };
      setIsReady(true);
    }
  }, [session_key, tenantsLoaded, campaignsLoaded, tenants.length, campaigns.length]);
  ...
  //     const tenantsChanged = currentTenants.length !== previousDataRef.current.tenants.length ||
  //       currentTenants.some((tenant, idx) => {
  //         const prev = previousDataRef.current.tenants[idx];
  //         return !prev || tenant.tenant_id !== prev.tenant_id || tenant.tenant_name !== prev.tenant_name;
  //       });

  //     const campaignsChanged = currentCampaigns.length !== previousDataRef.current.campaigns.length ||
  //       currentCampaigns.some((campaign, idx) => {
  //         const prev = previousDataRef.current.campaigns[idx];
  //         return !prev ||
  //           campaign.campaign_id !== prev.campaign_id ||
  //           campaign.campaign_name !== prev.campaign_name ||
  //           campaign.start_flag !== prev.start_flag;
  //       });
  ...
  const generateTreeMenuData = async (): Promise<TabData[]> => {
    ...
    try {
      const skillData = await fetchskillCampaignList();
      const campaignSkillsMap: Record<string, any[]> = {};

      skillData?.result_data?.forEach((skill: any) => {
        skill?.campaign_id?.forEach((campaignId: string) => {
          if (!campaignSkillsMap[campaignId]) campaignSkillsMap[campaignId] = [];
          const exists = campaignSkillsMap[campaignId].some(s => s.skill_id === skill.skill_id);
          if (!exists) campaignSkillsMap[campaignId].push({ skill_id: skill.skill_id, tenant_id: skill.tenant_id });
        });
      });
