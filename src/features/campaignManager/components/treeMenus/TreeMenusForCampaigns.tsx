...
//     return items.map(item => {
//       // NEXUS 루트 노드의 경우 자식들을 처리
//       if (item.id === "nexus") {
//         // nodeType이 'all' 또는 'tenant'인 경우 테넌트 노드 정렬
...
//             if (sortType === 'name') {
//               return sortFactor * a.label.localeCompare(b.label, 'ko');
//             } else if (sortType === 'id') {
...
//         return {
//           ...item,
//           children: sortedChildren.map(tenantNode => {
//             if (tenantNode.children) {
//               // nodeType이 'all' 또는 'campaign'인 경우에만 캠페인 노드 정렬
//               if (nodeType === 'all' || nodeType === 'campaign') {
//                 const campaignChildren = tenantNode.children.filter(child => child.type === "campaign");
//                 const otherChildren = tenantNode.children.filter(child => child.type !== "campaign");
...
//                 return {
//                   ...tenantNode,
//                   children: [...sortedCampaigns, ...otherChildren]
//                 };
//               }
//             }
//             return tenantNode;
...
//   // 기존 sortCampaignsWithinFolder 대신 개선된 sortTreeItems 함수 사용
...
//     viewMode,
//     selectedNodeType // 노드 타입 변경 시에도 재측정
//   ]);
...
export function TreeMenusForCampaigns() {
  ...
  // 필터링 로직
  function filterTreeItems(items: TreeItem[]): TreeItem[] {
    // 필터 모드가 'all'이면 뷰 모드에 따라 적절히 필터링
    if (filterMode === "all") {
      // 전체 모드이면서 테넌트 뷰인 경우만 테넌트 필터링 적용
      if (selectedNodeType === 'tenant' || (selectedNodeType !== 'all' && viewMode === 'tenant')) {
        return filterByViewMode(items, 'tenant');
      }
      // 전체 모드이지만 캠페인 뷰인 경우 필터링 없음
      return items;
    }
    ...
    // 전체 모드가 아닌 경우 뷰 모드에 따라 추가 필터링
    if (selectedNodeType === 'tenant' || (selectedNodeType !== 'all' && viewMode === 'tenant')) {
      return filterByViewMode(filteredItems, 'tenant');
    }
    return filteredItems;
  }

  // 뷰 모드에 따른 필터링 (테넌트만 또는 캠페인까지)
  function filterByViewMode(items: TreeItem[], mode: ViewMode): TreeItem[] {
    if (mode === 'tenant') {
      return items.map(item => {
        if (item.type === "folder") {
          // 폴더 내의 테넌트 노드만 유지하고 캠페인 노드는 제외
  ...
  useEffect(() => {
    if (!isLoading && !error && treeData && treeData.length > 0) {
      ...
      // 선택된 노드 타입과 뷰 모드에 따라 다른 레벨까지 확장
      let maxExpandLevel = 1; // 기본값: 테넌트 모드
      
      if (selectedNodeType === 'all') {
        // 전체 모드일 때는 캠페인까지 모두 표시
        maxExpandLevel = 3;
      } else if (selectedNodeType === 'campaign' || viewMode === 'campaign') {
        // 캠페인 모드일 때
        maxExpandLevel = 3;
      }
  ...
  }, [isLoading, error, treeData, expandNodes, originalItems, viewMode, selectedNodeType]);
