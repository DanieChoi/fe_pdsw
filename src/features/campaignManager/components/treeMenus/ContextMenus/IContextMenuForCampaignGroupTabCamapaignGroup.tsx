...
const IContextMenuForCampaignGroupTabCamapaignGroup: FC<IContextMenuForCampaignGroupTabCamapaignProps> = ({
  ...
  const { addTabCurrentOnly } = useTabStore();
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [isRenameDialogOpen, setIsRenameDialogOpen] = useState(false);
  ...
  return (
    <>
      ...
      <Item onClick={() => console.log(`캠페인 그룹 일괄 시작: ${node.name}`)}>
        캠페인 그룹 일괄 시작
      </Item>
      <Item onClick={() => console.log(`캠페인 그룹 일괄 완료: ${node.name}`)}>
        캠페인 그룹 일괄 완료
      </Item>
      <Item onClick={() => console.log(`캠페인 그룹 일괄 중지: ${node.name}`)}>
        캠페인 그룹 일괄 중지
      </Item>
      <Item
        onClick={() => {
      ...
      <Item onClick={() => console.log(`캠페인 그룹 실시간 재발신: ${node.name}`)}>
        캠페인 그룹 실시간 재발신
      </Item>
      <Separator />
      <CommonDialogForSideMenu
