...
export default function Footer({
  ...
  // 이벤트 코드 형식화 함수
  const formatEventCode = (id: string | number) => {
  ...
  const footerDataSet = useCallback((announce: string, command: string, data: any, kind: string, tempEventData: any): void => {
    ...
    //캠페인.
      ...
      if (data['start_flag'] === 3) {
        ...
        // 알림 설정이 활성화되어 있으면 토스트 표시
        if (useAlramPopup === 1) {
          toast.event(`[EVENT]${formatEventCode(data['campaign_id'])} 캠페인 상태 변경`, {
            duration: 6000,
          });
        }
      }
    ...
    //스킬.
      ...
      if (useAlramPopup === 1) {
        tempFooterDataList.forEach(item => {
          toast.event(`[EVENT]${formatEventCode(_skillId)} 상담원 스킬 ${command === 'DELETE' ? '해제' : '할당'}`, {
            duration: 6000
          });
        });
      }
...
//캠페인 동작상태 변경
  ...
  if (command === 'UPDATE') {
    ...
    _message2 = `[EVENT]${formatEventCode(data['campaign_id'])} 캠페인 상태 변경`;
    
    // 토스트 메시지를 위한 이벤트 코드 포맷팅
    const eventCode = formatEventCode(data['campaign_id']);
    ...
    //발신리스트등록
      ...
      if (command === 'INSERT') {
        ...
        _message += '캠페인 아이디 : ' + data['campaign_id'] + ' , 리스트구분 : ' + list_flag;
        _message2 = `[EVENT]${formatEventCode(data['campaign_id'])} 발신리스트 ${list_flag} 등록`;
